<script lang="ts">
  ...
  async function saveDraft() {
    ...
    try {
      const saved = await invoke<LabProfile>('save_lab_draft', { draft });
      draft = undefined;
      await refresh();
      onSaved(saved);
  ...
  async function restoreDefault() {
    ...
    try {
      ...
      await refresh();
      onSaved(restored);
      onStatus(`Restored the shipped course default as new active revision ${restored.profile_version}.`);
    } catch (reason) { error = String(reason); }
  }
...
{#if visible}
  <div class="backdrop">
    <dialog open class="lab-dialog" aria-modal="true" aria-labelledby="lab-manager-title">
      <div class="manager-heading"><div><h2 id="lab-manager-title">Manage Labs</h2><p class="help">Local instructor workflow guard only. A saved lab must still be supported by the connected controlled firmware before recording.</p></div><button type="button" onclick={() => { visible = false; draft = undefined; }}>Close</button></div>
...
<style>
  ...
  button { min-height: 2.4rem; border: 1px solid #002855; border-radius: .3rem; background: #fff; color: #002855; padding: .45rem .7rem; font: inherit; font-weight: 650; cursor: pointer; } button.gold { background: #EEAA00; color: #17222e; } button:focus-visible, input:focus-visible, select:focus-visible, textarea:focus-visible { outline: 3px solid #EEAA00; outline-offset: 2px; }
